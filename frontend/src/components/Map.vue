    ...
    <div class="target">
      ...
      <div class="target-about">
        <div class="target-text">
          Для работы приложения необходимо активировать службу геолокации на вашем телефоне. Также необходимо разрешить
          доступ к геолокации вашему браузеру.
        </div>
...
export default {
  ...
  methods: {
    ...
    async handleMarkerClick(target) {
      ...
      if (this.playerCoords[0] - 0.001 < parseFloat(target.latitude) && this.playerCoords[0] + 0.001 > parseFloat(target.latitude) &&
          this.playerCoords[1] - 0.001 < parseFloat(target.longitude) && this.playerCoords[1] + 0.001 > parseFloat(target.longitude)) {
        this.activeTarget = {
          ...await response.json(),
          onPoint: true,
        };
