...
export default {
  ...
  methods: {
    ...
    async handleMarkerClick(target) {
      ...
      if (this.playerCoords[0] - 0.1015 < parseFloat(target.latitude) && this.playerCoords[0] + 0.1015 > parseFloat(target.latitude) &&
          this.playerCoords[1] - 0.1015 < parseFloat(target.longitude) && this.playerCoords[1] + 0.1015 > parseFloat(target.longitude)) {
        this.activeTarget = {
          ...await response.json(),
          onPoint: true,
        };
